...
<body class="offline-page bg-light">
    ...
    <?php if ($offlineDate): 
        $offlineFinishedText = Text::_('TPL_MINIMALISTA_OFFLINE_FINISHED');
        $countdownScript = "
            document.addEventListener('DOMContentLoaded', function() {
                function updateCountdown() {
                    var countdownEl = document.getElementById('countdown');
                    if (!countdownEl) return;
                    var targetDate = new Date(countdownEl.dataset.date).getTime();
                    var now = new Date().getTime();
                    var distance = targetDate - now;

                    if (distance < 0) {
                        countdownEl.innerHTML = \"<h3 class='text-primary fw-bold'>{$offlineFinishedText}</h3>\";
                        return;
                    }
        ...
        ";
        $this->getWebAssetManager()->addInlineScript($countdownScript);
    endif; ?>
</body>
